 ...
 * PageRank.java
 * @author Aditi Goel
 * Email: agoel4@uncc.edu
 * 
*/

package org.myorg;

//importing packages needed for necessary operations to perform
...
public class PageRank extends Configured implements Tool {
    ...
    public int run(String[] args) throws Exception {
        ...
        //Instantiated & configured job1 to count the number of links
        Configuration conf1 = new Configuration();

		 // set the output and intermediate path  and directories
        ...
        Job job1 = Job.getInstance(conf1, "CountNumLines");//setting the job configuration
        job1.setJarByClass(PageRank.class); //setting the jar class 

        FileInputFormat.addInputPaths(job1, args[0]);//setting i/p from 1st argument
        FileOutputFormat.setOutputPath(job1, links_count);//o/p for 2nd argument
...
//Instantiated & configured job2 to perform Map & Reduce for computing link graph
            int num_nodes = 1;
            int pr = 1;
			...
			// get the instance of job 2
            Job job2 = Job.getInstance(conf2, "CalculateLinkGraph");//setting the job configuration
            job2.setJarByClass(PageRank.class);//setting the jar class

            FileInputFormat.addInputPaths(job2, args[0]);//setting i/p from 1st argument
            FileOutputFormat.setOutputPath(job2, link_graph_input);//o/p for 2nd argument
            ...
            // The total nodes available inside job2 to use for initial page rank
...
/*
*I nstantiated & configured job3 to perform Map & Reduce for computing 
* page rank called iteratively for 10 times
 */
                ...
                for (int iteration = 1; iteration < 11; iteration++) {
					...
					// get the instance of job 3
                    Job job3 = Job.getInstance(conf3, "CalculatePageRank");//setting the job configuration
                    job3.setJarByClass(PageRank.class); //setting the jar class

                    System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>PAGERANK: ITERATION " + iteration);
                    Path intermed = new Path(intermediate_path, "file" + iteration);

                    FileInputFormat.addInputPath(job3, link_graph_input);//setting i/p from 1st argument
                    FileOutputFormat.setOutputPath(job3, intermed);//o/p for 2nd argument
                    ...
                    Path ip = new Path(intermediate_path + "/" + "file" + iteration);
                    // hdfs.delete(ip, true);
                ...
                if (pr == 0) {
					...
					//Instantiated & configured the job4 variable to perform Map & Reduce jobs for sorting page ranks
                    Configuration conf4 = new Configuration();

					// get the instance of job 4
                    Job job4 = Job.getInstance(conf4, "sortPageRank");//setting the job configuration
                    job4.setJarByClass(PageRank.class);//setting the jar class

                    System.out.println(">>>>>>>>>>>>>>>>>>>>>>>> SORTING <<<<<<<<<<<<<<<<<<<");

                    FileInputFormat.addInputPath(job4, link_graph_input);//setting i/p from 1st argument
                    FileOutputFormat.setOutputPath(job4, output_path);//o/p for 2nd argument
                    ...
                    int sort = job4.waitForCompletion(true) ? 0 : 1;
...
/* -------------------------------------------------------------------------------------*/
    ...
    /*
    * Mapper-1
    *This MapReduce is used to count the number of input lines in the file
   ...
   */
    ...
    public static class MapCount extends Mapper<LongWritable, Text, Text, IntWritable> {
        ...
        public void map(LongWritable offset, Text lineText, Context context)
            ...
            if (lineText != null) {
                if (lineText.toString().trim().length() > 0)//converted to String and trimmed
   ...
   */
    ...
    public static class ReduceCount extends Reducer<Text, IntWritable, Text, IntWritable> {
        ...
        public void reduce(Text key, Iterable<IntWritable> values, Context context)
            ...
            context.write(key, new IntWritable(sum)); //calculate number of lines in file
...
/* -------------------------------------------------------------------------------------*/
   ...
   */
    ...
    public static class MapLinkGraph extends Mapper<LongWritable ,  Text ,  Text ,  Text > {
        ...
        final Pattern title = Pattern .compile("<title>(.+?)</title>"); //identify content within title
        final Pattern outlinks = Pattern .compile("<text(.+?)</text>"); //identify content within text
        final Pattern brackets = Pattern.compile("\\[\\[.*?]\\]"); //Identify Regex pattern
